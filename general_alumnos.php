...
if(isset($_POST['search'])){
    ...
    $f_desde = $_POST['desde'];
    $f_hasta = $_POST['hasta'];
    $sql = "(select * from alumnos where no_control like '%".$ncontrol."%' and fecha between '".$f_desde."' and '".$f_hasta."')";
    $resultado = $conn->query($sql);
    if ($f_desde == "") {
        if ($f_hasta == "") {
            $sql = "(select * from alumnos where no_control like '%".$ncontrol."%')";
            $resultado = $conn->query($sql);
        }
    }
    if ($ncontrol == "") {
        $sql = "(select * from alumnos where fecha between '".$f_desde."' and '".$f_hasta."')";
            $resultado = $conn->query($sql);
    }
    if ($ncontrol == ""){
        if ($f_desde == "") {
