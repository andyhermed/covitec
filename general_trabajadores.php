...
if(isset($_POST['search'])){
    ...
    $f_desde = $_POST['desde'];
    $f_hasta = $_POST['hasta'];
    $sql = "(select * from trabajadores no_checador like '%".$nchecador."%' and fecha between '".$f_desde."' and '".$f_hasta."')";
    $resultado = $conn->query($sql);
    if ($f_desde == "") {
        if ($f_hasta == "") {
            $sql = "(select * from trabajadores no_checador like '%".$nchecador."%')";
            $resultado = $conn->query($sql);
        }
    }
    if ($nchecador == "") {
        $sql = "(select * from trabajadores where fecha between '".$f_desde."' and '".$f_hasta."')";
            $resultado = $conn->query($sql);
    }
    if ($nchecador == ""){
        if ($f_desde == "") {
